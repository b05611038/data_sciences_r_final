...
class GrabNow():
    def __init__(self, save_dir = './data', url = 'http://tisvcloud.freeway.gov.tw/'):
        ...
        self.sub_title = ['roadlevel_info.xml.gz', 'roadlevel_value.xml.gz', 'roadlevel_value5.xml.gz', 'vd_info.xml.gz', 'vd_value.xml.gz', 'vd_value5.xml.gz']
    ...
    def grab(self):
        for i in range(len(self.sub_title)):
            ...
            save_file = self.save_dir + '/' + self.sub_title[i]
            wget.download(download_url, out = save_file)
